...
const Banner = ({ name, description, to, showEditButton=false }) => {
    ...
    return (
        <Link to={to} className="w-full block">
            <div className="relative flex flex-col sm:flex-row items-center p-4 sm:p-9 
                bg-white border-l-4 border-blue-600 rounded-lg shadow-md 
                hover:shadow-lg transition-shadow duration-200">
                <button
                    onClick={handleFollow}
                    className={`absolute top-2 ${showEditButton ? 'right-20' : 'right-2'} z-10 
                        px-4 py-2 sm:px-3 sm:py-1.5 
                        rounded-full text-sm font-medium transition-all duration-200 
                        flex items-center gap-2 min-h-[44px] sm:min-h-[32px]
                        touch-manipulation
                        ${isFollowed 
