...
export default function UserAccount({ isOpen, onClose, onLogout, username = 'User', buttonRef }) {
  ...
  useEffect(() => {
    ...
    setUserInfo({
      ...
      email: storedEmail || 'No email available' // Provide a fallback message
    });
  }, [username, isOpen]); // Add isOpen to dependencies to refresh on modal open
  ...
  // Update handleLogout to properly clear email
  const handleLogout = () => {
    localStorage.removeItem('username');
    localStorage.removeItem('email'); // Make sure email is removed
    localStorage.removeItem('isLoggedIn');
    onLogout();
    onClose();

    setUserInfo({ username: 'User', email: 'No email available' }); // Reset user info
